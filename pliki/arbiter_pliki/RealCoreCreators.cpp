...
namespace arbiter
{
    ...
    DefInsCreator::DefInsCreator(const unsigned int CORE_TO_CREATE_SIZE, const Warrior &warrior_1, const Warrior &warrior_2, const InsPtr &default_ins)
    : CoreCreator(CORE_TO_CREATE_SIZE, warrior_1, warrior_2), def_ins_(default_ins->clone())
    {}

    DefInsCreator::CorePtr DefInsCreator::createCore(Observer &obs_ptr) const
    {
        IntegerRegister FIRST_POS = countPositionWarrior1();//jesli rdzeń jest za mały dla wojowników - to te fcje rzucają wyjątek
        IntegerRegister SECOND_POD = countPositionWarrior2();
        ...
        for(unsigned int i = 0 ;i != FIRST_POS.getValue(); ++i)
        {
            created_core->core_memory_.push_back(def_ins_->clone() );
        }
    ...
    void DefInsCreator::modifyCore(CorePtr &existed_core_ptr) const
    {
        throw std::runtime_error("TODO");
    }
}
